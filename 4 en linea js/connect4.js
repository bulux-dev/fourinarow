    ...
    methods: {
        ...
        async askUserGameMode() {
            ...
            const result = await Swal.fire({
                title: 'Bienvenido',
                text: "Escoge tu modo de juego",
                showCancelButton: true,
                confirmButtonColor: '#gray',
                cancelButtonColor: '#blue',
                cancelButtonText: 'j1 vs j2',
                confirmButtonText: 'j1 Vs CPU'
            });
            this.canPlay = true;
        ...
        selectPlayer() {
            if (this.getRandomNumberBetween(0, 1) === 0) {
                alert("Inicia jugador 1");
                this.currentPlayer = PLAYER_1;
            } else {
                alert("Inicia jugador 2");
        ...
        async makeMove(columnNumber) {
            ...
            if (firstEmptyRow === -1) {
                Swal.fire('Escoge otra fila, esta esta llena    ');
                return;
            }
        ...
        async showWinner() {
            if (this.currentPlayer === PLAYER_1) {
                await Swal.fire('J1 gana');
            } else {
                await Swal.fire('j2 gana');
            }
        },
        async showTie() {
            await Swal.fire('Empate');
        },
